recipes = {
  "small_electric_pole": {
    "id": "small_electric_pole",
    "name": "Small electric pole",
    "resultCount": 2.0,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["wood", 2.0],
      ["copper_cable", 2.0]
    ]
  },
  "gun_turret": {
    "id": "gun_turret",
    "name": "Gun turret",
    "resultCount": 1,
    "speed": 0.2,
    "category": "crafting",
    "ingredients": [
      ["iron_gear_wheel", 5.0],
      ["copper_plate", 5.0],
      ["iron_plate", 10.0]
    ]
  },
  "processing_unit": {
    "id": "processing_unit",
    "name": "Processing Unit",
    "resultCount": 1,
    "speed": 0.06666666666666667,
    "category": "crafting-with-fluid",
    "ingredients": [
      ["electronic_circuit", 20.0],
      ["advanced_circuit", 2.0],
      ["sulfuric_acid", 0.5]
    ]
  },
  "radar": {
    "id": "radar",
    "name": "Radar",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["electronic_circuit", 5.0],
      ["iron_gear_wheel", 5.0],
      ["iron_plate", 10.0]
    ]
  },
  "diesel_locomotive": {
    "id": "diesel_locomotive",
    "name": "Diesel Locomotive",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["engine_unit", 15.0],
      ["electronic_circuit", 5.0],
      ["steel_plate", 10.0]
    ]
  },
  "stone_furnace": {
    "id": "stone_furnace",
    "name": "Stone Furnace",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["stone", 5.0]
    ]
  },
  "car": {
    "id": "car",
    "name": "Car",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["engine_unit", 8.0],
      ["iron_plate", 20.0],
      ["steel_plate", 5.0]
    ]
  },
  "pipe": {
    "id": "pipe",
    "name": "Pipe",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["iron_plate", 1.0]
    ]
  },
  "solid_fuel_from_light_oil": {
    "id": "solid_fuel_from_light_oil",
    "name": "solid fuel from light oil",
    "resultCount": 1,
    "speed": 0.3333333333333333,
    "category": "chemistry",
    "ingredients": [
      ["light_oil", 1.0]
    ]
  },
  "copper_cable": {
    "id": "copper_cable",
    "name": "Copper cable",
    "resultCount": 2.0,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["copper_plate", 1.0]
    ]
  },
  "cargo_wagon": {
    "id": "cargo_wagon",
    "name": "Cargo wagon",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["iron_gear_wheel", 10.0],
      ["iron_plate", 20.0],
      ["steel_plate", 5.0]
    ]
  },
  "wood": {
    "id": "wood",
    "name": "Wood",
    "resultCount": 2.0,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["raw_wood", 1.0]
    ]
  },
  "rocket": {
    "id": "rocket",
    "name": "Rocket",
    "resultCount": 1,
    "speed": 0.125,
    "category": "crafting",
    "ingredients": [
      ["electronic_circuit", 1.0],
      ["explosives", 2.0],
      ["iron_plate", 2.0]
    ]
  },
  "big_electric_pole": {
    "id": "big_electric_pole",
    "name": "Big electric pole",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["steel_plate", 5.0],
      ["copper_plate", 5.0]
    ]
  },
  "chemical_plant": {
    "id": "chemical_plant",
    "name": "Chemical plant",
    "resultCount": 1,
    "speed": 0.1,
    "category": "crafting",
    "ingredients": [
      ["steel_plate", 5.0],
      ["iron_gear_wheel", 5.0],
      ["electronic_circuit", 5.0],
      ["pipe", 5.0]
    ]
  },
  "roboport": {
    "id": "roboport",
    "name": "Roboport",
    "resultCount": 1,
    "speed": 0.06666666666666667,
    "category": "crafting",
    "ingredients": [
      ["steel_plate", 45.0],
      ["iron_gear_wheel", 45.0],
      ["advanced_circuit", 45.0]
    ]
  },
  "basic_transport_belt": {
    "id": "basic_transport_belt",
    "name": "Transport Belt",
    "resultCount": 2.0,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["iron_plate", 1.0],
      ["iron_gear_wheel", 1.0]
    ]
  },
  "heavy_armor": {
    "id": "heavy_armor",
    "name": "Heavy armor",
    "resultCount": 1,
    "speed": 0.125,
    "category": "crafting",
    "ingredients": [
      ["copper_plate", 100.0],
      ["steel_plate", 50.0]
    ]
  },
  "empty_barrel": {
    "id": "empty_barrel",
    "name": "Empty Barrel",
    "resultCount": 1,
    "speed": 1.0,
    "category": "crafting",
    "ingredients": [
      ["steel_plate", 1.0]
    ]
  },
  "light_oil_cracking": {
    "id": "light_oil_cracking",
    "name": "light oil cracking",
    "resultCount": 1,
    "speed": 0.2,
    "category": "chemistry",
    "ingredients": [
      ["light_oil", 3.0],
      ["water", 3.0]
    ]
  },
  "fast_splitter": {
    "id": "fast_splitter",
    "name": "Fast Splitter",
    "resultCount": 1,
    "speed": 0.5,
    "category": "crafting",
    "ingredients": [
      ["electronic_circuit", 10.0],
      ["iron_gear_wheel", 10.0],
      ["basic_splitter", 1.0]
    ]
  },
  "basic_electric_discharge_defense_equipment": {
    "id": "basic_electric_discharge_defense_equipment",
    "name": "Discharge defense",
    "resultCount": 1,
    "speed": 0.1,
    "category": "crafting",
    "ingredients": [
      ["processing_unit", 5.0],
      ["steel_plate", 20.0],
      ["laser_turret", 10.0]
    ]
  },
  "rocket_defense": {
    "id": "rocket_defense",
    "name": "Rocket defense",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["rocket", 100.0],
      ["advanced_circuit", 128.0],
      ["processing_unit", 128.0],
      ["speed_module_3", 50.0],
      ["productivity_module_3", 50.0]
    ]
  },
  "productivity_module": {
    "id": "productivity_module",
    "name": "Productivity module",
    "resultCount": 1,
    "speed": 0.06666666666666667,
    "category": "crafting",
    "ingredients": [
      ["advanced_circuit", 5.0],
      ["electronic_circuit", 5.0]
    ]
  },
  "offshore_pump": {
    "id": "offshore_pump",
    "name": "Offshore Pump",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["electronic_circuit", 2.0],
      ["pipe", 1.0],
      ["iron_gear_wheel", 1.0]
    ]
  },
  "storage_tank": {
    "id": "storage_tank",
    "name": "Storage Tank",
    "resultCount": 1,
    "speed": 0.3333333333333333,
    "category": "crafting",
    "ingredients": [
      ["iron_plate", 20.0],
      ["steel_plate", 5.0]
    ]
  },
  "night_vision_equipment": {
    "id": "night_vision_equipment",
    "name": "Night vision",
    "resultCount": 1,
    "speed": 0.1,
    "category": "crafting",
    "ingredients": [
      ["advanced_circuit", 5.0],
      ["steel_plate", 10.0]
    ]
  },
  "steel_axe": {
    "id": "steel_axe",
    "name": "Steel axe",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["steel_plate", 5.0],
      ["iron_stick", 2.0]
    ]
  },
  "destroyer_capsule": {
    "id": "destroyer_capsule",
    "name": "Destroyer capsule",
    "resultCount": 1,
    "speed": 0.06666666666666667,
    "category": "crafting",
    "ingredients": [
      ["distractor_capsule", 4.0],
      ["speed_module", 1.0]
    ]
  },
  "energy_shield_mk2_equipment": {
    "id": "energy_shield_mk2_equipment",
    "name": "Energy shield MK2",
    "resultCount": 1,
    "speed": 0.1,
    "category": "crafting",
    "ingredients": [
      ["energy_shield_equipment", 10.0],
      ["processing_unit", 10.0]
    ]
  },
  "basic_bullet_magazine": {
    "id": "basic_bullet_magazine",
    "name": "Regular Magazine",
    "resultCount": 1.0,
    "speed": 0.5,
    "category": "crafting",
    "ingredients": [
      ["iron_plate", 2.0]
    ]
  },
  "logistic_chest_passive_provider": {
    "id": "logistic_chest_passive_provider",
    "name": "Passive Provider Chest",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["smart_chest", 1.0],
      ["advanced_circuit", 1.0]
    ]
  },
  "power_armor_mk2": {
    "id": "power_armor_mk2",
    "name": "Power armor MK2",
    "resultCount": 1,
    "speed": 0.04,
    "category": "crafting",
    "ingredients": [
      ["effectivity_module_3", 5.0],
      ["speed_module_3", 5.0],
      ["processing_unit", 200.0],
      ["steel_plate", 50.0],
      ["alien_artifact", 50.0]
    ]
  },
  "curved_rail": {
    "id": "curved_rail",
    "name": "Curved Rail",
    "resultCount": 2.0,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["stone", 4.0],
      ["iron_stick", 4.0],
      ["steel_plate", 4.0]
    ]
  },
  "substation": {
    "id": "substation",
    "name": "Substation",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["steel_plate", 10.0],
      ["advanced_circuit", 5.0],
      ["copper_plate", 5.0]
    ]
  },
  "basic_beacon": {
    "id": "basic_beacon",
    "name": "Basic Beacon",
    "resultCount": 1,
    "speed": 0.06666666666666667,
    "category": "crafting",
    "ingredients": [
      ["electronic_circuit", 20.0],
      ["advanced_circuit", 20.0],
      ["steel_plate", 10.0],
      ["copper_cable", 10.0]
    ]
  },
  "poison_capsule": {
    "id": "poison_capsule",
    "name": "Poison capsule",
    "resultCount": 1,
    "speed": 0.125,
    "category": "crafting",
    "ingredients": [
      ["steel_plate", 3.0],
      ["electronic_circuit", 3.0],
      ["coal", 10.0]
    ]
  },
  "basic_mining_drill": {
    "id": "basic_mining_drill",
    "name": "Electric Mining Drill",
    "resultCount": 1,
    "speed": 0.5,
    "category": "crafting",
    "ingredients": [
      ["electronic_circuit", 3.0],
      ["iron_gear_wheel", 5.0],
      ["iron_plate", 10.0]
    ]
  },
  "shotgun_shell": {
    "id": "shotgun_shell",
    "name": "Shotgun shells",
    "resultCount": 1,
    "speed": 0.3333333333333333,
    "category": "crafting",
    "ingredients": [
      ["copper_plate", 2.0],
      ["iron_plate", 2.0]
    ]
  },
  "basic_accumulator": {
    "id": "basic_accumulator",
    "name": "Basic accumulator",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["iron_plate", 2.0],
      ["battery", 5.0]
    ]
  },
  "red_wire": {
    "id": "red_wire",
    "name": "Red wire",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["electronic_circuit", 1.0],
      ["copper_cable", 1.0]
    ]
  },
  "assembling_machine_3": {
    "id": "assembling_machine_3",
    "name": "Assembling machine 3",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["speed_module", 4.0],
      ["assembling_machine_2", 2.0]
    ]
  },
  "heavy_oil_cracking": {
    "id": "heavy_oil_cracking",
    "name": "heavy oil cracking",
    "resultCount": 1,
    "speed": 0.2,
    "category": "chemistry",
    "ingredients": [
      ["heavy_oil", 4.0],
      ["water", 3.0]
    ]
  },
  "lubricant": {
    "id": "lubricant",
    "name": "Lubricant",
    "resultCount": 1,
    "speed": 1.0,
    "category": "chemistry",
    "ingredients": [
      ["heavy_oil", 1.0]
    ]
  },
  "productivity_module_2": {
    "id": "productivity_module_2",
    "name": "Productivity module 2",
    "resultCount": 1,
    "speed": 0.03333333333333333,
    "category": "crafting",
    "ingredients": [
      ["productivity_module", 4.0],
      ["advanced_circuit", 5.0],
      ["processing_unit", 5.0]
    ]
  },
  "oil_refinery": {
    "id": "oil_refinery",
    "name": "Oil refinery",
    "resultCount": 1,
    "speed": 0.05,
    "category": "crafting",
    "ingredients": [
      ["steel_plate", 15.0],
      ["iron_gear_wheel", 10.0],
      ["stone_brick", 10.0],
      ["electronic_circuit", 10.0],
      ["pipe", 10.0]
    ]
  },
  "basic_splitter": {
    "id": "basic_splitter",
    "name": "Splitter",
    "resultCount": 1,
    "speed": 1.0,
    "category": "crafting",
    "ingredients": [
      ["electronic_circuit", 5.0],
      ["iron_plate", 5.0],
      ["basic_transport_belt", 4.0]
    ]
  },
  "burner_mining_drill": {
    "id": "burner_mining_drill",
    "name": "Burner Mining Drill",
    "resultCount": 1,
    "speed": 0.5,
    "category": "crafting",
    "ingredients": [
      ["iron_gear_wheel", 3.0],
      ["stone_furnace", 1.0],
      ["iron_plate", 3.0]
    ]
  },
  "wall": {
    "id": "wall",
    "name": "Wall",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["stone_brick", 5.0]
    ]
  },
  "assembling_machine_1": {
    "id": "assembling_machine_1",
    "name": "Assembling machine 1",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["electronic_circuit", 3.0],
      ["iron_gear_wheel", 5.0],
      ["iron_plate", 9.0]
    ]
  },
  "logistic_chest_requester": {
    "id": "logistic_chest_requester",
    "name": "Requester Chest",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["smart_chest", 1.0],
      ["advanced_circuit", 1.0]
    ]
  },
  "green_wire": {
    "id": "green_wire",
    "name": "Green wire",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["electronic_circuit", 1.0],
      ["copper_cable", 1.0]
    ]
  },
  "effectivity_module": {
    "id": "effectivity_module",
    "name": "Effectivity module",
    "resultCount": 1,
    "speed": 0.06666666666666667,
    "category": "crafting",
    "ingredients": [
      ["advanced_circuit", 5.0],
      ["electronic_circuit", 5.0]
    ]
  },
  "steel_plate": {
    "id": "steel_plate",
    "name": "Steel Plate",
    "resultCount": 1,
    "speed": 0.05714285714285714,
    "category": "smelting",
    "ingredients": [
      ["iron_plate", 5.0]
    ]
  },
  "solid_fuel_from_heavy_oil": {
    "id": "solid_fuel_from_heavy_oil",
    "name": "solid fuel from heavy oil",
    "resultCount": 1,
    "speed": 0.3333333333333333,
    "category": "chemistry",
    "ingredients": [
      ["heavy_oil", 2.0]
    ]
  },
  "speed_module_2": {
    "id": "speed_module_2",
    "name": "Speed module 2",
    "resultCount": 1,
    "speed": 0.03333333333333333,
    "category": "crafting",
    "ingredients": [
      ["speed_module", 4.0],
      ["processing_unit", 5.0],
      ["advanced_circuit", 5.0]
    ]
  },
  "laser_turret": {
    "id": "laser_turret",
    "name": "Laser Turret",
    "resultCount": 1,
    "speed": 0.2,
    "category": "crafting",
    "ingredients": [
      ["steel_plate", 5.0],
      ["electronic_circuit", 5.0],
      ["battery", 3.0]
    ]
  },
  "boiler": {
    "id": "boiler",
    "name": "Boiler",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["stone_furnace", 1.0],
      ["pipe", 1.0]
    ]
  },
  "construction_robot": {
    "id": "construction_robot",
    "name": "Construction robot",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["flying_robot_frame", 1.0],
      ["electronic_circuit", 2.0]
    ]
  },
  "rail_signal": {
    "id": "rail_signal",
    "name": "Rail signal",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["electronic_circuit", 1.0],
      ["iron_plate", 5.0]
    ]
  },
  "express_splitter": {
    "id": "express_splitter",
    "name": "Express Splitter",
    "resultCount": 1,
    "speed": 0.5,
    "category": "crafting",
    "ingredients": [
      ["advanced_circuit", 10.0],
      ["iron_gear_wheel", 10.0],
      ["fast_splitter", 1.0],
      ["lubricant", 2.0]
    ]
  },
  "plastic_bar": {
    "id": "plastic_bar",
    "name": "Plastic bar",
    "resultCount": 1,
    "speed": 1.0,
    "category": "chemistry",
    "ingredients": [
      ["petroleum_gas", 3.0],
      ["coal", 1.0]
    ]
  },
  "electric_furnace": {
    "id": "electric_furnace",
    "name": "Electric Furnace",
    "resultCount": 1,
    "speed": 0.2,
    "category": "crafting",
    "ingredients": [
      ["steel_plate", 15.0],
      ["advanced_circuit", 5.0],
      ["stone_brick", 10.0]
    ]
  },
  "steel_furnace": {
    "id": "steel_furnace",
    "name": "Steel Furnace",
    "resultCount": 1,
    "speed": 0.3333333333333333,
    "category": "crafting",
    "ingredients": [
      ["steel_plate", 8.0],
      ["stone_brick", 10.0]
    ]
  },
  "pistol": {
    "id": "pistol",
    "name": "Pistol",
    "resultCount": 1,
    "speed": 1.0,
    "category": "crafting",
    "ingredients": [
      ["copper_plate", 5.0],
      ["iron_plate", 5.0]
    ]
  },
  "express_transport_belt_to_ground": {
    "id": "express_transport_belt_to_ground",
    "name": "Express underground belt",
    "resultCount": 2.0,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["iron_gear_wheel", 40.0],
      ["fast_transport_belt_to_ground", 2.0],
      ["lubricant", 4.0]
    ]
  },
  "iron_axe": {
    "id": "iron_axe",
    "name": "Iron axe",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["iron_stick", 2.0],
      ["iron_plate", 3.0]
    ]
  },
  "small_plane": {
    "id": "small_plane",
    "name": "Small plane",
    "resultCount": 1,
    "speed": 0.03333333333333333,
    "category": "crafting",
    "ingredients": [
      ["plastic_bar", 120.0],
      ["advanced_circuit", 250.0],
      ["electric_engine_unit", 20.0],
      ["battery", 150.0]
    ]
  },
  "express_transport_belt": {
    "id": "express_transport_belt",
    "name": "Express Transport Belt",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting-with-fluid",
    "ingredients": [
      ["iron_gear_wheel", 5.0],
      ["fast_transport_belt", 1.0],
      ["lubricant", 2.0]
    ]
  },
  "fast_transport_belt": {
    "id": "fast_transport_belt",
    "name": "Fast Transport Belt",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["iron_gear_wheel", 5.0],
      ["basic_transport_belt", 1.0]
    ]
  },
  "advanced_circuit": {
    "id": "advanced_circuit",
    "name": "Advanced Circuit",
    "resultCount": 1,
    "speed": 0.125,
    "category": "crafting",
    "ingredients": [
      ["electronic_circuit", 2.0],
      ["plastic_bar", 2.0],
      ["copper_cable", 4.0]
    ]
  },
  "shotgun": {
    "id": "shotgun",
    "name": "Shotgun",
    "resultCount": 1,
    "speed": 0.25,
    "category": "crafting",
    "ingredients": [
      ["iron_plate", 15.0],
      ["iron_gear_wheel", 5.0],
      ["copper_plate", 10.0],
      ["wood", 5.0]
    ]
  },
  "sulfuric_acid": {
    "id": "sulfuric_acid",
    "name": "Sulfuric acid",
    "resultCount": 1,
    "speed": 1.0,
    "category": "chemistry",
    "ingredients": [
      ["sulfur", 5.0],
      ["iron_plate", 1.0],
      ["water", 10.0]
    ]
  },
  "basic_inserter": {
    "id": "basic_inserter",
    "name": "Inserter",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["electronic_circuit", 1.0],
      ["iron_gear_wheel", 1.0],
      ["iron_plate", 1.0]
    ]
  },
  "solar_panel_equipment": {
    "id": "solar_panel_equipment",
    "name": "Portable solar panel",
    "resultCount": 1,
    "speed": 0.1,
    "category": "crafting",
    "ingredients": [
      ["solar_panel", 5.0],
      ["processing_unit", 1.0],
      ["steel_plate", 5.0]
    ]
  },
  "electronic_circuit": {
    "id": "electronic_circuit",
    "name": "Electronic Circuit",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["iron_plate", 1.0],
      ["copper_cable", 3.0]
    ]
  },
  "pipe_to_ground": {
    "id": "pipe_to_ground",
    "name": "Pipe-to-Ground",
    "resultCount": 2.0,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["pipe", 10.0],
      ["iron_plate", 5.0]
    ]
  },
  "train_stop": {
    "id": "train_stop",
    "name": "Train stop",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["electronic_circuit", 5.0],
      ["iron_plate", 10.0],
      ["steel_plate", 3.0]
    ]
  },
  "battery_mk2_equipment": {
    "id": "battery_mk2_equipment",
    "name": "Battery MK2",
    "resultCount": 1,
    "speed": 0.1,
    "category": "crafting",
    "ingredients": [
      ["battery_equipment", 10.0],
      ["processing_unit", 20.0]
    ]
  },
  "explosives": {
    "id": "explosives",
    "name": "Explosives",
    "resultCount": 1,
    "speed": 0.2,
    "category": "chemistry",
    "ingredients": [
      ["sulfur", 1.0],
      ["coal", 1.0],
      ["water", 1.0]
    ]
  },
  "slowdown_capsule": {
    "id": "slowdown_capsule",
    "name": "Slowdown capsule",
    "resultCount": 1,
    "speed": 0.125,
    "category": "crafting",
    "ingredients": [
      ["steel_plate", 2.0],
      ["electronic_circuit", 2.0],
      ["coal", 5.0]
    ]
  },
  "land_mine": {
    "id": "land_mine",
    "name": "Land mine",
    "resultCount": 4.0,
    "speed": 0.2,
    "category": "crafting",
    "ingredients": [
      ["steel_plate", 1.0],
      ["explosives", 2.0]
    ]
  },
  "basic_armor": {
    "id": "basic_armor",
    "name": "Iron armor",
    "resultCount": 1,
    "speed": 0.3333333333333333,
    "category": "crafting",
    "ingredients": [
      ["iron_plate", 40.0]
    ]
  },
  "combat_shotgun": {
    "id": "combat_shotgun",
    "name": "Combat shotgun",
    "resultCount": 1,
    "speed": 0.125,
    "category": "crafting",
    "ingredients": [
      ["steel_plate", 15.0],
      ["iron_gear_wheel", 5.0],
      ["copper_plate", 10.0],
      ["wood", 10.0]
    ]
  },
  "assembling_machine_2": {
    "id": "assembling_machine_2",
    "name": "Assembling machine 2",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["iron_plate", 9.0],
      ["electronic_circuit", 3.0],
      ["iron_gear_wheel", 5.0],
      ["assembling_machine_1", 1.0]
    ]
  },
  "pumpjack": {
    "id": "pumpjack",
    "name": "Pumpjack",
    "resultCount": 1,
    "speed": 0.05,
    "category": "crafting",
    "ingredients": [
      ["steel_plate", 15.0],
      ["iron_gear_wheel", 10.0],
      ["electronic_circuit", 10.0],
      ["pipe", 10.0]
    ]
  },
  "flying_robot_frame": {
    "id": "flying_robot_frame",
    "name": "Flying robot frame",
    "resultCount": 1,
    "speed": 0.05,
    "category": "crafting",
    "ingredients": [
      ["electric_engine_unit", 1.0],
      ["battery", 2.0],
      ["steel_plate", 1.0],
      ["electronic_circuit", 3.0]
    ]
  },
  "smart_chest": {
    "id": "smart_chest",
    "name": "Smart Chest",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["steel_chest", 1.0],
      ["electronic_circuit", 3.0]
    ]
  },
  "deconstruction_planner": {
    "id": "deconstruction_planner",
    "name": "Deconstruction planner",
    "resultCount": 1,
    "speed": 1.0,
    "category": "crafting",
    "ingredients": [
      ["advanced_circuit", 1.0]
    ]
  },
  "basic_transport_belt_to_ground": {
    "id": "basic_transport_belt_to_ground",
    "name": "Underground belt",
    "resultCount": 2.0,
    "speed": 1.0,
    "category": "crafting",
    "ingredients": [
      ["iron_plate", 10.0],
      ["basic_transport_belt", 5.0]
    ]
  },
  "fast_inserter": {
    "id": "fast_inserter",
    "name": "Fast inserter",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["electronic_circuit", 2.0],
      ["iron_plate", 2.0],
      ["basic_inserter", 1.0]
    ]
  },
  "explosive_rocket": {
    "id": "explosive_rocket",
    "name": "Explosive rocket",
    "resultCount": 1,
    "speed": 0.125,
    "category": "crafting",
    "ingredients": [
      ["rocket", 1.0],
      ["explosives", 5.0]
    ]
  },
  "distractor_capsule": {
    "id": "distractor_capsule",
    "name": "Distractor capsule",
    "resultCount": 1,
    "speed": 0.06666666666666667,
    "category": "crafting",
    "ingredients": [
      ["defender_capsule", 4.0],
      ["advanced_circuit", 3.0]
    ]
  },
  "smart_inserter": {
    "id": "smart_inserter",
    "name": "Smart inserter",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["fast_inserter", 1.0],
      ["electronic_circuit", 4.0]
    ]
  },
  "fusion_reactor_equipment": {
    "id": "fusion_reactor_equipment",
    "name": "Portable fusion reactor",
    "resultCount": 1,
    "speed": 0.1,
    "category": "crafting",
    "ingredients": [
      ["processing_unit", 100.0],
      ["alien_artifact", 30.0]
    ]
  },
  "iron_gear_wheel": {
    "id": "iron_gear_wheel",
    "name": "Iron Gear Wheel",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["iron_plate", 2.0]
    ]
  },
  "basic_electric_discharge_defense_remote": {
    "id": "basic_electric_discharge_defense_remote",
    "name": "Basic electric discharge defense remote",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["electronic_circuit", 1.0]
    ]
  },
  "speed_module": {
    "id": "speed_module",
    "name": "Speed module",
    "resultCount": 1,
    "speed": 0.06666666666666667,
    "category": "crafting",
    "ingredients": [
      ["advanced_circuit", 5.0],
      ["electronic_circuit", 5.0]
    ]
  },
  "burner_inserter": {
    "id": "burner_inserter",
    "name": "Burner Inserter",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["iron_plate", 1.0],
      ["iron_gear_wheel", 1.0]
    ]
  },
  "logistic_chest_storage": {
    "id": "logistic_chest_storage",
    "name": "Storage Chest",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["smart_chest", 1.0],
      ["advanced_circuit", 1.0]
    ]
  },
  "steel_chest": {
    "id": "steel_chest",
    "name": "Steel Chest",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["steel_plate", 8.0]
    ]
  },
  "submachine_gun": {
    "id": "submachine_gun",
    "name": "Submachine gun",
    "resultCount": 1,
    "speed": 0.3333333333333333,
    "category": "crafting",
    "ingredients": [
      ["iron_gear_wheel", 10.0],
      ["copper_plate", 5.0],
      ["iron_plate", 10.0]
    ]
  },
  "stone_brick": {
    "id": "stone_brick",
    "name": "Stone Brick",
    "resultCount": 1,
    "speed": 0.2857142857142857,
    "category": "smelting",
    "ingredients": [
      ["stone", 2.0]
    ]
  },
  "power_armor": {
    "id": "power_armor",
    "name": "Power armor",
    "resultCount": 1,
    "speed": 0.05,
    "category": "crafting",
    "ingredients": [
      ["processing_unit", 100.0],
      ["electric_engine_unit", 30.0],
      ["steel_plate", 100.0],
      ["alien_artifact", 10.0]
    ]
  },
  "fast_transport_belt_to_ground": {
    "id": "fast_transport_belt_to_ground",
    "name": "Fast underground belt",
    "resultCount": 2.0,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["iron_gear_wheel", 20.0],
      ["basic_transport_belt_to_ground", 2.0]
    ]
  },
  "flame_thrower_ammo": {
    "id": "flame_thrower_ammo",
    "name": "Flamethrower Ammo",
    "resultCount": 1,
    "speed": 0.3333333333333333,
    "category": "chemistry",
    "ingredients": [
      ["iron_plate", 5.0],
      ["light_oil", 2.5],
      ["heavy_oil", 2.5]
    ]
  },
  "basic_modular_armor": {
    "id": "basic_modular_armor",
    "name": "Basic modular armor",
    "resultCount": 1,
    "speed": 0.06666666666666667,
    "category": "crafting",
    "ingredients": [
      ["advanced_circuit", 30.0],
      ["processing_unit", 5.0],
      ["steel_plate", 50.0]
    ]
  },
  "straight_rail": {
    "id": "straight_rail",
    "name": "Straight Rail",
    "resultCount": 2.0,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["stone", 1.0],
      ["iron_stick", 1.0],
      ["steel_plate", 1.0]
    ]
  },
  "medium_electric_pole": {
    "id": "medium_electric_pole",
    "name": "Medium electric pole",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["steel_plate", 2.0],
      ["copper_plate", 2.0]
    ]
  },
  "engine_unit": {
    "id": "engine_unit",
    "name": "Engine unit",
    "resultCount": 1,
    "speed": 0.05,
    "category": "advanced-crafting",
    "ingredients": [
      ["steel_plate", 1.0],
      ["iron_gear_wheel", 1.0],
      ["pipe", 2.0]
    ]
  },
  "electric_engine_unit": {
    "id": "electric_engine_unit",
    "name": "Electric engine unit",
    "resultCount": 1,
    "speed": 0.05,
    "category": "crafting-with-fluid",
    "ingredients": [
      ["engine_unit", 1.0],
      ["lubricant", 2.0],
      ["electronic_circuit", 2.0]
    ]
  },
  "flame_thrower": {
    "id": "flame_thrower",
    "name": "Flamethrower",
    "resultCount": 1,
    "speed": 0.1,
    "category": "crafting",
    "ingredients": [
      ["steel_plate", 5.0],
      ["iron_gear_wheel", 10.0]
    ]
  },
  "advanced_oil_processing": {
    "id": "advanced_oil_processing",
    "name": "advanced oil processing",
    "resultCount": 1,
    "speed": 0.2,
    "category": "oil-processing",
    "ingredients": [
      ["crude_oil", 10.0],
      ["water", 5.0]
    ]
  },
  "productivity_module_3": {
    "id": "productivity_module_3",
    "name": "Productivity module 3",
    "resultCount": 1,
    "speed": 0.016666666666666666,
    "category": "crafting",
    "ingredients": [
      ["productivity_module_2", 5.0],
      ["advanced_circuit", 5.0],
      ["processing_unit", 5.0],
      ["alien_artifact", 1.0]
    ]
  },
  "copper_plate": {
    "id": "copper_plate",
    "name": "Copper Plate",
    "resultCount": 1,
    "speed": 0.2857142857142857,
    "category": "smelting",
    "ingredients": [
      ["copper_ore", 1.0]
    ]
  },
  "effectivity_module_3": {
    "id": "effectivity_module_3",
    "name": "Effectivity module 3",
    "resultCount": 1,
    "speed": 0.016666666666666666,
    "category": "crafting",
    "ingredients": [
      ["effectivity_module_2", 5.0],
      ["advanced_circuit", 5.0],
      ["processing_unit", 5.0],
      ["alien_artifact", 1.0]
    ]
  },
  "solar_panel": {
    "id": "solar_panel",
    "name": "Solar Panel",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["steel_plate", 5.0],
      ["electronic_circuit", 15.0],
      ["copper_plate", 5.0]
    ]
  },
  "logistic_chest_active_provider": {
    "id": "logistic_chest_active_provider",
    "name": "Active Provider Chest",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["smart_chest", 1.0],
      ["advanced_circuit", 1.0]
    ]
  },
  "basic_exoskeleton_equipment": {
    "id": "basic_exoskeleton_equipment",
    "name": "Basic exoskeleton equipment",
    "resultCount": 1,
    "speed": 0.1,
    "category": "crafting",
    "ingredients": [
      ["processing_unit", 10.0],
      ["electric_engine_unit", 30.0],
      ["steel_plate", 20.0]
    ]
  },
  "railgun": {
    "id": "railgun",
    "name": "Railgun",
    "resultCount": 1,
    "speed": 0.125,
    "category": "crafting",
    "ingredients": [
      ["steel_plate", 15.0],
      ["copper_plate", 15.0],
      ["electronic_circuit", 10.0],
      ["advanced_circuit", 5.0]
    ]
  },
  "fill_crude_oil_barrel": {
    "id": "fill_crude_oil_barrel",
    "name": "fill crude oil barrel",
    "resultCount": 1,
    "speed": 1.0,
    "category": "crafting-with-fluid",
    "ingredients": [
      ["crude_oil", 25.0],
      ["empty_barrel", 1.0]
    ]
  },
  "logistic_robot": {
    "id": "logistic_robot",
    "name": "Logistic robot",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["flying_robot_frame", 1.0],
      ["advanced_circuit", 2.0]
    ]
  },
  "blueprint": {
    "id": "blueprint",
    "name": "Blueprint",
    "resultCount": 1,
    "speed": 1.0,
    "category": "crafting",
    "ingredients": [
      ["advanced_circuit", 1.0]
    ]
  },
  "repair_pack": {
    "id": "repair_pack",
    "name": "Repair pack",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["electronic_circuit", 1.0],
      ["iron_gear_wheel", 1.0]
    ]
  },
  "science_pack_1": {
    "id": "science_pack_1",
    "name": "Science Pack 1",
    "resultCount": 1,
    "speed": 0.2,
    "category": "crafting",
    "ingredients": [
      ["copper_plate", 1.0],
      ["iron_gear_wheel", 1.0]
    ]
  },
  "defender_capsule": {
    "id": "defender_capsule",
    "name": "Defender capsule",
    "resultCount": 1,
    "speed": 0.125,
    "category": "crafting",
    "ingredients": [
      ["piercing_bullet_magazine", 1.0],
      ["electronic_circuit", 2.0],
      ["iron_gear_wheel", 3.0]
    ]
  },
  "speed_module_3": {
    "id": "speed_module_3",
    "name": "Speed module 3",
    "resultCount": 1,
    "speed": 0.016666666666666666,
    "category": "crafting",
    "ingredients": [
      ["speed_module_2", 4.0],
      ["advanced_circuit", 5.0],
      ["processing_unit", 5.0],
      ["alien_artifact", 1.0]
    ]
  },
  "empty_crude_oil_barrel": {
    "id": "empty_crude_oil_barrel",
    "name": "empty crude oil barrel",
    "resultCount": 1,
    "speed": 1.0,
    "category": "crafting-with-fluid",
    "ingredients": [
      ["crude_oil_barrel", 1.0]
    ]
  },
  "energy_shield_equipment": {
    "id": "energy_shield_equipment",
    "name": "Energy shield",
    "resultCount": 1,
    "speed": 0.1,
    "category": "crafting",
    "ingredients": [
      ["advanced_circuit", 5.0],
      ["steel_plate", 10.0]
    ]
  },
  "basic_oil_processing": {
    "id": "basic_oil_processing",
    "name": "basic oil processing",
    "resultCount": 1,
    "speed": 0.2,
    "category": "oil-processing",
    "ingredients": [
      ["crude_oil", 10.0]
    ]
  },
  "long_handed_inserter": {
    "id": "long_handed_inserter",
    "name": "Long handed inserter",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["iron_gear_wheel", 1.0],
      ["iron_plate", 1.0],
      ["basic_inserter", 1.0]
    ]
  },
  "alien_science_pack": {
    "id": "alien_science_pack",
    "name": "Alien science pack",
    "resultCount": 10.0,
    "speed": 0.08333333333333333,
    "category": "crafting",
    "ingredients": [
      ["alien_artifact", 1.0]
    ]
  },
  "piercing_bullet_magazine": {
    "id": "piercing_bullet_magazine",
    "name": "Piercing Rounds Magazine",
    "resultCount": 1,
    "speed": 0.3333333333333333,
    "category": "crafting",
    "ingredients": [
      ["copper_plate", 5.0],
      ["steel_plate", 1.0]
    ]
  },
  "rocket_launcher": {
    "id": "rocket_launcher",
    "name": "Rocket launcher",
    "resultCount": 1,
    "speed": 0.2,
    "category": "crafting",
    "ingredients": [
      ["iron_plate", 5.0],
      ["iron_gear_wheel", 5.0],
      ["electronic_circuit", 5.0]
    ]
  },
  "piercing_shotgun_shell": {
    "id": "piercing_shotgun_shell",
    "name": "Piercing shotgun shells",
    "resultCount": 1,
    "speed": 0.125,
    "category": "crafting",
    "ingredients": [
      ["copper_plate", 2.0],
      ["steel_plate", 2.0]
    ]
  },
  "battery": {
    "id": "battery",
    "name": "Battery",
    "resultCount": 1,
    "speed": 0.2,
    "category": "chemistry",
    "ingredients": [
      ["sulfuric_acid", 2.0],
      ["iron_plate", 1.0],
      ["copper_plate", 1.0]
    ]
  },
  "small_pump": {
    "id": "small_pump",
    "name": "Small Pump",
    "resultCount": 1,
    "speed": 0.5,
    "category": "crafting",
    "ingredients": [
      ["electric_engine_unit", 1.0],
      ["steel_plate", 1.0],
      ["pipe", 1.0]
    ]
  },
  "basic_grenade": {
    "id": "basic_grenade",
    "name": "Basic grenade",
    "resultCount": 1,
    "speed": 0.125,
    "category": "crafting",
    "ingredients": [
      ["iron_plate", 5.0],
      ["coal", 10.0]
    ]
  },
  "basic_laser_defense_equipment": {
    "id": "basic_laser_defense_equipment",
    "name": "Personal laser defense",
    "resultCount": 1,
    "speed": 0.1,
    "category": "crafting",
    "ingredients": [
      ["processing_unit", 1.0],
      ["steel_plate", 5.0],
      ["laser_turret", 5.0]
    ]
  },
  "small_lamp": {
    "id": "small_lamp",
    "name": "Lamp",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["electronic_circuit", 1.0],
      ["iron_stick", 3.0],
      ["iron_plate", 1.0]
    ]
  },
  "railgun_dart": {
    "id": "railgun_dart",
    "name": "Railgun darts",
    "resultCount": 1,
    "speed": 0.125,
    "category": "crafting",
    "ingredients": [
      ["steel_plate", 5.0],
      ["electronic_circuit", 5.0]
    ]
  },
  "solid_fuel_from_petroleum_gas": {
    "id": "solid_fuel_from_petroleum_gas",
    "name": "solid fuel from petroleum gas",
    "resultCount": 1,
    "speed": 0.3333333333333333,
    "category": "chemistry",
    "ingredients": [
      ["petroleum_gas", 2.0]
    ]
  },
  "steam_engine": {
    "id": "steam_engine",
    "name": "Steam engine",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["iron_gear_wheel", 5.0],
      ["pipe", 5.0],
      ["iron_plate", 5.0]
    ]
  },
  "iron_plate": {
    "id": "iron_plate",
    "name": "Iron Plate",
    "resultCount": 1,
    "speed": 0.2857142857142857,
    "category": "smelting",
    "ingredients": [
      ["iron_ore", 1.0]
    ]
  },
  "iron_chest": {
    "id": "iron_chest",
    "name": "Iron Chest",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["iron_plate", 8.0]
    ]
  },
  "science_pack_2": {
    "id": "science_pack_2",
    "name": "Science Pack 2",
    "resultCount": 1,
    "speed": 0.16666666666666666,
    "category": "crafting",
    "ingredients": [
      ["basic_inserter", 1.0],
      ["basic_transport_belt", 1.0]
    ]
  },
  "wooden_chest": {
    "id": "wooden_chest",
    "name": "Wooden chest",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["wood", 4.0]
    ]
  },
  "science_pack_3": {
    "id": "science_pack_3",
    "name": "Science Pack 3",
    "resultCount": 1,
    "speed": 0.08333333333333333,
    "category": "crafting",
    "ingredients": [
      ["battery", 1.0],
      ["advanced_circuit", 1.0],
      ["smart_inserter", 1.0],
      ["steel_plate", 1.0]
    ]
  },
  "player_port": {
    "id": "player_port",
    "name": "Player port",
    "resultCount": 1,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["electronic_circuit", 10.0],
      ["iron_gear_wheel", 5.0],
      ["iron_plate", 1.0]
    ]
  },
  "lab": {
    "id": "lab",
    "name": "Lab",
    "resultCount": 1,
    "speed": 0.2,
    "category": "crafting",
    "ingredients": [
      ["electronic_circuit", 10.0],
      ["iron_gear_wheel", 10.0],
      ["basic_transport_belt", 4.0]
    ]
  },
  "iron_stick": {
    "id": "iron_stick",
    "name": "Iron Stick",
    "resultCount": 2.0,
    "speed": 2,
    "category": "crafting",
    "ingredients": [
      ["iron_plate", 1.0]
    ]
  },
  "effectivity_module_2": {
    "id": "effectivity_module_2",
    "name": "Effectivity module 2",
    "resultCount": 1,
    "speed": 0.03333333333333333,
    "category": "crafting",
    "ingredients": [
      ["effectivity_module", 4.0],
      ["advanced_circuit", 5.0],
      ["processing_unit", 5.0]
    ]
  },
  "sulfur": {
    "id": "sulfur",
    "name": "Sulfur",
    "resultCount": 1,
    "speed": 1.0,
    "category": "chemistry",
    "ingredients": [
      ["petroleum_gas", 3.0],
      ["water", 3.0]
    ]
  },
  "battery_equipment": {
    "id": "battery_equipment",
    "name": "Battery",
    "resultCount": 1,
    "speed": 0.1,
    "category": "crafting",
    "ingredients": [
      ["battery", 5.0],
      ["steel_plate", 10.0]
    ]
  }
};
